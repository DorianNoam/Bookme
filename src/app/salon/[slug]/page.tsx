...
export default async function SalonPage({
  ...
  if (isNumeric) {
    ...
    // URL avec slug : chercher par slug
    const { data } = await supabase.from('salons').select('*').eq('slug', slugParam).single()
    salonData = data
  }
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', background: BG, minHeight: '100vh', paddingBottom: 80 }}>
      ...
      {isGalleryOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(10,10,10,0.98)', zIndex: 9999, overflowY: 'auto', padding: '60px 20px' }}>
          <Link href={`/salon/${salon.slug || salonId}`} scroll={false} style={{ position: 'fixed', top: 20, right: 30, color: '#fff', fontSize: 40, textDecoration: 'none', fontWeight: 300, zIndex: 10000, width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.1)', borderRadius: '50%' }}>
            &times;
          </Link>
          ...
          <div>
            ...
            <div style={{ marginBottom: 32 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
               ...
               <div style={{ flexShrink: 0 }}><FavoriteButton salonId={String(salonId)} /></div>
              </div>
              ...
              <div style={{ marginBottom: 32 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 8 }}>
                  {safeGallery.map((img: any, i: number) => (
                    <Link key={img.id} href={`/salon/${salon.slug || salonId}?gallery=open`} scroll={false} style={{ display: 'block', borderRadius: 8, overflow: 'hidden', height: 160, border: '1px solid #E0D8CE' }}>
