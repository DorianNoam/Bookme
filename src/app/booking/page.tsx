...
type Service = { 
  ...
  categorie_service?: string; description?: string;
}
...
function BookingContent() {
  ...
  async function fetchCreneaux(d: string) {
    ...
    try {
      ...
      params.set('service_id', String(service.id))
      params.set('date', d)
  ...
  function handleSelectService(s: Service) {
    setService(s)
    router.push(`/booking?salon=${salonId}&service=${s.id}`)
  }
  ...
  async function handleConfirm() {
    ...
    try {
      const res = await fetch('/api/reservations', {
        ...
        body: JSON.stringify({ salon_id: parseInt(salonId!), service_id: service!.id, employe_id: selectedCreneau!.emp_id, date_rdv: date + 'T' + selectedCreneau!.heure + ':00', client_nom: clientNom.trim(), client_prenom: clientPrenom.trim(), client_email: clientEmail.trim(), client_telephone: clientTelephone.trim() }),
      })
      const data = await res.json()
      ...
      <div style={{ maxWidth: 600, margin: '40px auto', padding: '0 16px', textAlign: 'center' }}>
        <div style={{ background: '#fff', border: '1px solid #EDE5D8', borderRadius: 6, padding: '40px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            {loggedUser && (
              <Link href="/dashboard" style={{ display: 'inline-block', background: OR, color: '#fff', padding: '12px 24px', borderRadius: 4, fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>Mes rendez-vous</Link>
            )}
            <Link href="/search" style={{ display: 'inline-block', background: NOIR, color: '#fff', padding: '12px 24px', borderRadius: 4, fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>Retour a la recherche</Link>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', background: BG, minHeight: '100vh', paddingBottom: 60 }}>
      ...
      <header style={{ background: '#fff', borderBottom: '1px solid #F0EAE0', padding: '12px 0', marginBottom: 20 }}>
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Link href="/" style={{ fontSize: 20, fontWeight: 900, color: NOIR, textDecoration: 'none' }}>Bookme<span style={{ color: OR }}>dz</span></Link>
          <Link href={'/salon/' + salonId} style={{ color: '#777', fontSize: 13, textDecoration: 'none' }}>{'\u2190 Retour au salon'}</Link>
        </div>
      </header>

      <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 16px' }}>

        {/* CHOIX DU SERVICE SI PAS PRE-SELECTIONNE */}
        {!service && (
          <div style={{ background: '#fff', border: '1px solid #EDE5D8', borderRadius: 6, padding: '24px 20px', marginBottom: 20 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: NOIR, marginBottom: 6 }}>Choisissez une prestation</h2>
            <p style={{ color: '#888', fontSize: 13, marginBottom: 20 }}>{salon?.nom} — {salon?.ville}</p>
            {allServices.length === 0 ? (
              <p style={{ color: '#999', fontSize: 14, textAlign: 'center', padding: 20 }}>Aucune prestation disponible.</p>
            ) : (
              <div style={{ display: 'grid', gap: 8 }}>
                {allServices.map((s: Service) => {
                  const hasP = s.promo_active && s.promo_pourcentage && s.promo_pourcentage > 0
                  const pPrice = hasP ? Math.round(s.prix - (s.prix * s.promo_pourcentage! / 100)) : s.prix
                  return (
                    <button key={s.id} onClick={() => handleSelectService(s)}
                      style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', background: '#fff', border: '1px solid #E0D8CE', borderRadius: 8, cursor: 'pointer', fontFamily: 'Inter, sans-serif', textAlign: 'left', transition: 'border-color 0.2s', gap: 12 }}
                      onMouseEnter={e => (e.currentTarget.style.borderColor = OR)}
                      onMouseLeave={e => (e.currentTarget.style.borderColor = '#E0D8CE')}
                    >
                      <div>
                        <div style={{ fontWeight: 700, color: NOIR, fontSize: 15, marginBottom: 4 }}>{s.nom}</div>
                        {s.description && <div style={{ color: '#aaa', fontSize: 12, marginBottom: 4 }}>{s.description}</div>}
                        <div style={{ color: '#888', fontSize: 13 }}>{s.duree} min</div>
                      </div>
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        {hasP ? (
                          <div>
                            <div style={{ textDecoration: 'line-through', color: '#aaa', fontSize: 12 }}>{s.prix} DA</div>
                            <div style={{ fontWeight: 900, color: '#d32f2f', fontSize: 16 }}>{pPrice} DA</div>
                          </div>
                        ) : (
                          <div style={{ fontWeight: 800, color: OR, fontSize: 16 }}>{s.prix} DA</div>
                        )}
                      </div>
                    </button>
                  )
        ...
        {/* CONTENU PRINCIPAL - UNIQUEMENT SI SERVICE SELECTIONNE */}
        {service && (
          <>
            ...
            {/* Bouton changer de prestation */}
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              <button onClick={() => { setService(null); setStep(1); setDate(''); setSelectedCreneau(null); router.push(`/booking?salon=${salonId}`) }} style={{ background: 'none', border: 'none', color: OR, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif', textDecoration: 'underline' }}>
                ...
                {loggedUser ? (
                  ...
                  <div>
                    <div style={{ marginBottom: 20, textAlign: 'center', padding: '16px', background: '#FDFBF7', borderRadius: 4, border: `1px dashed ${OR}40` }}>
                      ...
                      <Link href={`/login?redirect=/booking?salon=${salonId}&service=${service.id}`} style={{ display: 'inline-block', background: OR, color: '#fff', padding: '10px 20px', borderRadius: 4, fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>Se connecter</Link>
