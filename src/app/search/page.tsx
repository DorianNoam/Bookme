...
const CATEGORIES = ['Coiffure', 'Manucure', 'Institut', 'Massage', 'Barbier', 'Spa']
    ...
    <div style={{ background: '#f9f9f9', minHeight: '100vh' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', gap: 20 }}>
          ...
          <form onSubmit={handleSearch} style={{ flex: 1, display: 'flex', gap: 10 }}>
            <select
              ...
              style={{ flex: 1, padding: '10px 15px', border: '1px solid #ddd', borderRadius: 6, fontSize: 14, background: 'white', fontFamily: 'Inter, sans-serif' }}
            >
            ...
            <input
              ...
              placeholder="Ville (Alger, Oran...)"
              style={{ flex: 1, padding: '10px 15px', border: '1px solid #ddd', borderRadius: 6, fontSize: 14, fontFamily: 'Inter, sans-serif' }}
            />
            <button type="submit" style={{ background: '#111', color: 'white', border: 'none', padding: '10px 25px', borderRadius: 6, fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>
              Rechercher
            </button>
          </form>
      ...
      <div style={{ background: 'white', borderBottom: '1px solid #eee', padding: '12px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', display: 'flex', gap: 10, overflowX: 'auto' }}>
          <button onClick={() => router.push('/search')} style={{ padding: '7px 18px', borderRadius: 20, border: '1px solid #ddd', background: !query ? '#111' : 'white', color: !query ? 'white' : '#555', fontWeight: 600, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'Inter, sans-serif' }}>
            Tous
          </button>
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => router.push('/search?q=' + cat)} style={{ padding: '7px 18px', borderRadius: 20, border: '1px solid #ddd', background: query === cat ? '#111' : 'white', color: query === cat ? 'white' : '#555', fontWeight: 600, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'Inter, sans-serif' }}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '30px 20px' }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 5 }}>
          {query ? query : 'Tous les salons'}{loc ? ' a ' + loc : ''}
        </h1>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 60, border: '1px dashed #ccc', borderRadius: 10 }}>
            ...
            <p style={{ color: '#888', marginBottom: 20 }}>Aucun salon ne correspond a votre recherche.</p>
            <Link href="/search" style={{ color: '#111', fontWeight: 700 }}>Voir tous les salons</Link>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
            {salons.map(salon => (
              <Link key={salon.id} href={'/salon/' + salon.id} style={{ textDecoration: 'none', color: 'inherit' }}>
                <div
                  style={{ background: 'white', borderRadius: 10, border: '1px solid #eee', display: 'flex', overflow: 'hidden', cursor: 'pointer' }}
                  ...
                  <div style={{ flex: 1, padding: '20px 25px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 5 }}>
                        ...
                        {salon.type_salon && (
                          <span style={{ background: '#f0f7ff', color: '#4DA6FF', fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 4 }}>
                            {salon.type_salon}
                          </span>
                        )}
                      </div>
                      ...
                      {salon.description && (
                        <p style={{ color: '#555', fontSize: 13, lineHeight: 1.5 }}>
                          {salon.description.length > 120 ? salon.description.substring(0, 120) + '...' : salon.description}
                        </p>
                      )}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 15 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
                        {salon.moy_note ? (
                          <span style={{ color: '#f59e0b', fontWeight: 700 }}>★ {salon.moy_note} <span style={{ color: '#888', fontWeight: 400, fontSize: 12 }}>({salon.nb_avis} avis)</span></span>
                        ) : (
                          <span style={{ color: '#888', fontSize: 12 }}>Nouveau salon</span>
                        )}
                        {salon.telephone && <span style={{ color: '#888', fontSize: 12 }}>📞 {salon.telephone}</span>}
                      </div>
                      <span style={{ background: '#111', color: 'white', padding: '8px 20px', borderRadius: 6, fontSize: 13, fontWeight: 700 }}>
                        Reserver →
                      </span>
                    </div>
                  </div>
