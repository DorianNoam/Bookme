    ...
    <div style={{ background: BG, minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      <header style={{ background: '#fff', borderBottom: '1px solid #F0EAE0', padding: '12px 0', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
          <Link href="/" style={{ fontSize: 20, fontWeight: 900, color: NOIR, whiteSpace: 'nowrap' }}>
          ...
          <form onSubmit={handleSearch} className="hide-mobile" style={{ flex: 1, display: 'flex', gap: 8 }}>
            ...
            <input value={loc} onChange={e => setLoc(e.target.value)} placeholder="Ville..." style={{ flex: 1, padding: '9px 12px', border: '1px solid #E0D8CE', borderRadius: 4, fontSize: 13, fontFamily: 'Inter, sans-serif', color: NOIR }} />
          ...
          <div className="hide-mobile" style={{ display: 'flex', gap: 10, whiteSpace: 'nowrap', alignItems: 'center' }}>
            {isLoggedIn ? (
              <Link href="/dashboard" style={{ background: NOIR, color: '#fff', padding: '8px 16px', borderRadius: 4, fontSize: 12, fontWeight: 700 }}>Mon espace</Link>
            ) : (
              <>
                <Link href="/login" style={{ color: '#555', fontSize: 13, fontWeight: 500 }}>Connexion</Link>
                <Link href="/register" style={{ background: NOIR, color: '#fff', padding: '8px 16px', borderRadius: 4, fontSize: 12, fontWeight: 700 }}>Inscription</Link>
              </>
            )}
          </div>
        </div>
        ...
        {showMobileSearch && (
          <div className="hide-desktop" style={{ padding: '12px 16px', borderTop: '1px solid #f0f0f0', background: '#fff' }}>
            <form onSubmit={handleSearch} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <input value={loc} onChange={e => setLoc(e.target.value)} placeholder="Ville (Alger, Oran...)" style={{ padding: '10px 12px', border: '1px solid #E0D8CE', borderRadius: 4, fontSize: 14, fontFamily: 'Inter, sans-serif', color: NOIR }} />
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 16px' }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: OR, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>Resultats</div>
        <h1 style={{ fontSize: 20, fontWeight: 800, marginBottom: 4, color: NOIR }}>{query ? query : 'Tous les etablissements'}{loc ? ' a ' + loc : ''}</h1>
        <p style={{ color: '#999', fontSize: 13, marginBottom: 24 }}>{loading ? 'Chargement...' : salons.length + ' etablissement(s)'}</p>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 40, background: '#fff', border: '1px dashed #DDD5C8', borderRadius: 4 }}>
            ...
            <p style={{ color: '#888', marginBottom: 16, fontSize: 14 }}>Aucun etablissement ne correspond.</p>
            <Link href="/search" style={{ color: OR, fontWeight: 700, borderBottom: '1px solid ' + OR, paddingBottom: 2, fontSize: 14 }}>Voir tous les etablissements</Link>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {salons.map(salon => (
              <Link key={salon.id} href={'/salon/' + salon.id} style={{ color: 'inherit' }}>
                <div className="salon-result-card" style={{ background: '#fff', borderRadius: 6, border: '1px solid #EDE5D8', overflow: 'hidden', cursor: 'pointer', transition: 'box-shadow 0.2s' }}
                  ...
                  <div style={{ flex: 1, padding: '18px 20px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12, flexWrap: 'wrap', gap: 8 }}>
                      ...
                      <span style={{ background: NOIR, color: '#fff', padding: '7px 18px', borderRadius: 3, fontSize: 12, fontWeight: 700 }}>Reserver {'→'}</span>
                    </div>
                  </div>
                </div>
              </Link>
