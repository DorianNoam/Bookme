...
const MapWithNoSSR = dynamic(() => import('@/components/SalonMap'), { 
  ssr: false,
  loading: () => <div style={{ height: 360, background: '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#666' }}>Chargement de la carte interactive...</div>
})
...
function SearchContent() {
  ...
  const [hoveredSalonId, setHoveredSalonId] = useState<number | null>(null) // Déclaration de l'état
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {salons.map(salon => (
              ...
              >
                <Link href={'/salon/' + salon.id} style={{ color: 'inherit', textDecoration: 'none' }}>
                  <div className="salon-result-card" style={{ background: '#fff', borderRadius: 6, border: '1px solid #EDE5D8', overflow: 'hidden', cursor: 'pointer', transition: 'box-shadow 0.2s' }}
                    onMouseEnter={e => (e.currentTarget.style.boxShadow = '0 4px 24px rgba(184,146,42,0.2)')}
